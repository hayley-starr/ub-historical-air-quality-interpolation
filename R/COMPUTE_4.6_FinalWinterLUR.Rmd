---
title: "Computing Final Winter LURS"
...
In this file:
1. Computing LUR for each Winter Month

Output:

File 'data/all_monthly_predictions.rds' with winter month predictions
...
show_predictor_table <- function(monthly_dataframe) {
  predictors <- monthly_dataframe %>% 
    ...
    #dplyr::select(-station_serial, -monthly_avg_pm25) %>%
    colnames()
...
```{r}
month_in_question <- '2019-12-01'
month_data_file <- paste0('data/monthly_avgs/month_of_', month_in_question, '.rds', sep='')

# ONE-OFF for special file -------------------------------------
#month_data_file <- 'data/monthly_avgs/month_of_end_feb_2019.rds'
...
```{r}
ols <- lm(monthly_avg_pm25 ~ res_road_005, dataframe)
tidy(ols)
...
```{r}
current_month_predictions <- predictions_at_stations %>% dplyr::select(station_serial, predicted) %>% mutate(date=date(month_in_question))
...
all_predictions <- all_predictions %>% filter(date != month_in_question)
